...
<div class="grid_column10">
	<h1 class="section-title section-title-center" style="display: flex; font-size: 1.5rem; color: #c038cb; webkit-box-align: center;">
		<b style="display: block; webkit-box-flex: 1; ms-flex: 1; flex: 1; height: 2px; opacity: .1; background-color: currentColor; margin: 14px;"></b>
			<span class="section-title-main">ĐỒ THỂ THAO KP SPORT</span>
		<b style="display: block; webkit-box-flex: 1; ms-flex: 1; flex: 1; height: 2px; opacity: .1; background-color: currentColor; margin: 14px;"></b>
	</h1>
	<?php
		$conn = mysqli_connect('localhost', 'root', '', 'sportshop') or die('Lỗi kết nối!'.mysqli_connect_error());
		$result = mysqli_query($conn, "select count(id_danhmuc) as total from sanpham");
	...
	<?php
		if(mysqli_num_rows($result) > 0){
			while($row = mysqli_fetch_assoc($result)){
	?>
				<form class="products" method="POST" action="noidung/themgiohang.php?id_sanpham=<?php echo $row_chitiet['id_sanpham']?>">
					<div class="product-info"> <a style="text-decoration: none;" href="index.php?quanly=chitiet&id=<?php echo $row['id_sanpham'] ?>">
						<img class="imgsp" src="<?php echo $row['hinhanh'] ?>"/>
						<h4 style="color: black;"> <strong> <?php echo $row['tensanpham'] ?></strong></h4>
...
<div class="pagination">
	<?php 			
		...
		if ($current_page > 1 && $total_page > 1){
			echo '<div class="page"> <a style="text-decoration:none;" href="index.php?page='.($current_page-1).' ">Prev</a> </div>';
		}
		// Lặp khoảng giữa
		...
		// ngược lại hiển thị thẻ a
			if ($i == $current_page){
				...
				echo '<div class="page"> <a style="text-decoration:none;" href="index.php?page='.$i.'">'.$i.' </a> </div>';
			}
		}
		// nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
		if ($current_page < $total_page && $total_page > 1){
			echo '<div class="page"> <a style="text-decoration:none;" href="index.php?page='.($current_page+1).' ">Next</a> </div>';
		}
	?>
</div>